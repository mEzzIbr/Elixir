...
import getSpaceReactions from "../api/getSpaceReactions";
import { useRouter } from "next/router";

const Home = () => {
    const [spaceReactions,SetSpaceReactions] = useState([])
    const router = useRouter();
    const LinkData = router.query;
    const SpaceId = LinkData.SpaceId
    console.log(SpaceId);
    if (SpaceId) {
    getSpaceReactions(SpaceId).then(data => {console.log(data)})    
    }
    
    return (
        <Layout>

<div className="text-sm font-medium text-center text-gray-500 border-b border-gray-200 dark:text-gray-400 dark:border-gray-700 w-10/12">
